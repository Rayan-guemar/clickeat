...
const db={exec:async s=>sql.exec(s),prepare(s){const statement=(values=[])=>({bind(...v){return statement(v)},first:async()=>sql.prepare(s).get(...values)||null,all:async()=>({results:sql.prepare(s).all(...values)}),run:async()=>sql.prepare(s).run(...values)});return statement()}};
...
const server=createServer(async(req,res)=>{try{
 ...
 const relative=['/','/client','/client/','/pilotage','/pilotage/'].includes(decodeURIComponent(url.pathname))?'index.html':decodeURIComponent(url.pathname).slice(1),path=resolve(root,relative);
