...
const worker=`${modules.join('\n')}\nlet handler;let boundDB;\nexport default {async fetch(request,env){if(new URL(request.url).pathname.startsWith('/api/')){if(!env.DB)return Response.json({error:'Database binding missing'},{status:503});if(boundDB!==env.DB){boundDB=env.DB;handler=api(env.DB);}return handler(request);}return env.ASSETS.fetch(request);}};\n`;
